...
def calculate_percentage_differences():
    """
    Calculate turn-by-turn averages of latency and ttft for each folder
    and calculate percentage differences compared to the baseline folder
    """
    ...
    for folder in folders:
        ...
        # Dictionary to store latency and ttft values for each turn
        turn_latencies = {}
        turn_ttfts = {}
    ...
    print("\nFolder Comparison          | Latency Diff | TTFT Diff | Generation Time Diff")
    print("-" * 78)
    ...
    for folder in folders[1:]:
        ...
        print(f"{folder:25s} | {avg_lat_diff:+11.2f}% | {avg_ttft_diff:+9.2f}% | {avg_gen_time_diff:+18.2f}%")

def plot_comparison_metrics():
    """
    Read CSV files from folders and generate comparison graphs 
    for Invocation Latency and TTFT
    """
    
    # Create two subplots (Invocation Latency, TTFT)
...
def plot_generation_time_comparison():
    """
    Generation Time (Latency - TTFT)
    """
    ...
    for idx, folder in enumerate(folders):
        ...
        plt.errorbar(turns, gen_time_means, yerr=gen_time_stds, 
    ...
    plt.title('Generation Time (Latency - TTFT) Comparison', fontsize=14)
    plt.legend()
    ...
    # Save and display graph
    plt.tight_layout()
    plt.savefig(f'generation_time_comparison_{date}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"\nGeneration Time graph saved as 'generation_time_comparison_{date}.png'.")
...
def main():
    ...
    # 0. Calculate percentage differences (newly added)
    calculate_percentage_differences()
    
    # 0-1. Calculate Generation Time differences (newly added)
    calculate_generation_time_differences()
    
    # 1. Invocation Latency and TTFT comparison graphs
    plot_comparison_metrics()
    
    # 2. Cache metrics comparison graphs
    plot_cache_metrics()
    
    # 3. Generation Time comparison graph (newly added)
    plot_generation_time_comparison()
